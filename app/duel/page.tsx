...
import { pageVariants, popVariants, slideUpVariants, fadeVariants } from "@/lib/animations";
...
export default function DuelPage() {
  ...
  return (
    <AnimatePresence mode="wait">
      {phase === "LOBBY" && (
        <motion.div key="LOBBY" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" style={{ padding: "20px", maxWidth: "600px", margin: "0 auto", paddingBottom: "100px", textAlign: "center" }}>
      ...
      {phase === "MATCHMAKING" && (
        <motion.div key="MATCHMAKING" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" style={{ height: "80vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ 
            ...
            borderRadius: "12px", width: "250px", textAlign: "center", boxShadow: "var(--neo-shadow)", overflow: "hidden"
          }}>
            <AnimatePresence mode="wait">
              <motion.span 
                key={opponentName}
                ...
                style={{ color: "var(--text-1)", fontWeight: 800, fontSize: "1.2rem", display: "inline-block" }}
              >
                {opponentName || "..."}
              </motion.span>
            </AnimatePresence>
      ...
      {phase === "COUNTDOWN" && (
        <motion.div key="COUNTDOWN" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" style={{ height: "80vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      {phase === "BATTLE" && currentWord && (
        <motion.div key="BATTLE" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" style={{ padding: "20px", maxWidth: "600px", margin: "0 auto", paddingBottom: "100px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px", backgroundColor: "var(--card-bg)", padding: "16px", borderRadius: "16px", border: "2.5px solid var(--border-color)", boxShadow: "var(--neo-shadow)" }}>
            <div style={{ textAlign: "center", width: "40%" }}>
              ...
              <motion.div key={playerScore} initial={{ scale: 1.3 }} animate={{ scale: 1 }} style={{ color: "#38E54D", fontSize: "2.5rem", fontWeight: 900 }}>{playerScore}</motion.div>
            </div>
            ...
            <div style={{ textAlign: "center", width: "40%" }}>
              ...
              <motion.div key={opponentScore} initial={{ scale: 1.3 }} animate={{ scale: 1 }} style={{ color: "#FF5964", fontSize: "2.5rem", fontWeight: 900 }}>{opponentScore}</motion.div>
            </div>
          </div>
          ...
          <div style={{ textAlign: "center", marginBottom: "40px" }}>
            ...
            <div style={{ width: "100%", height: "16px", backgroundColor: "var(--bg-base)", border: "2px solid var(--border-color)", borderRadius: "8px", overflow: "hidden", marginBottom: "24px" }}>
              <motion.div 
                initial={{ width: "100%" }}
                animate={{ width: `${(roundTimer / 8) * 100}%`, backgroundColor: roundTimer <= 3 ? "#FF5964" : "#4ECCD3" }}
            ...
            <AnimatePresence mode="wait">
              <motion.h2 
                ...
                style={{ fontSize: "3rem", fontWeight: 900, color: "var(--text-1)", textTransform: "capitalize", wordBreak: "break-word" }}
              >
                {currentWord.word}
              </motion.h2>
            </AnimatePresence>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            {options.map((opt, i) => (
              <motion.button
                ...
                whileHover={{ y: -2, boxShadow: "4px 6px 0 var(--border-color)" }}
                whileTap={{ scale: 0.95 }}
                style={{
                  ...
                  fontSize: "1.1rem", fontWeight: 800, cursor: "pointer", transition: "transform 0.08s ease, box-shadow 0.08s ease"
                }}
              >
                {opt}
              </motion.button>
      ...
      {phase === "RESULTS" && (
        <motion.div key="RESULTS" variants={popVariants} initial="hidden" animate="visible" exit="exit" style={{ padding: "20px", maxWidth: "600px", margin: "0 auto", textAlign: "center", paddingTop: "40px" }}>
          <div style={{ fontSize: "6rem", marginBottom: "20px" }}>
            <span style={{ fontFamily: "sans-serif, Apple Color Emoji, Segoe UI Emoji" }}>{playerScore > opponentScore ? "🏆" : playerScore === opponentScore ? "🤝" : "💀"}</span>
          </div>
          <h1 style={{ color: playerScore > opponentScore ? "#38E54D" : playerScore === opponentScore ? "#FFE052" : "#FF5964", fontSize: "3rem", fontWeight: 900, textTransform: "uppercase", margin: "0 0 24px 0", textShadow: "4px 4px 0 var(--border-color)" }}>
            {playerScore > opponentScore ? "CHIẾN THẮNG" : playerScore === opponentScore ? "HÒA" : "THẤT BẠI"}
          </h1>
          ...
          <div style={{ backgroundColor: "var(--card-bg)", padding: "32px", borderRadius: "16px", border: "4px solid var(--border-color)", boxShadow: "var(--neo-shadow)", marginBottom: "40px" }}>
            ...
            <div style={{ backgroundColor: "var(--bg-base)", padding: "16px", borderRadius: "12px", border: "2px solid var(--border-color)", display: "inline-flex", alignItems: "center", gap: "8px" }}>
              ...
              <span style={{ color: "#FFE052", fontWeight: 900, fontSize: "1.2rem", textShadow: "1px 1px 0 #111" }}>
                +{playerScore > opponentScore ? 150 : playerScore === opponentScore ? 75 : 30} XP
              </span>
            </div>
          </div>
